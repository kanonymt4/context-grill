...
- **未対応（MCP: 古いストアが `store` に居座る）**: `openStore()` が `await` の後で無条件に
  `store = s` するため、その間に `invalidateStore()` が走っても古いストアが再代入される。
  `IndexStore` のスナップショット化で例外は出なくなったが、**古い索引を引き続ける**点は
  未解消。索引に世代番号を持たせ、世代が変わっていたら代入を捨てる。
- **未対応（`open()` が検索しないコマンドでも postings を全部読む）**: スナップショット化の
  代償。実測 10,541 チャンクで 68ms / 25MB。`open(dir, { postings: false })` で避けられる。
- **未対応（`finish()` がアトミックでない）**: `writeFile` 6 回に分かれているため、書き込みの
  途中に `open()` すると新旧が混ざり得る。今回の変更は「開いた後」だけを守る。未検証。
