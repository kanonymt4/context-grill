...
- **未対応（MCP: 索引オープン中に sync が入ると古いストアが居座る）**: `openStore()` の実行中に
  `context_grill_sync` が来ると、`invalidateStore()` が `store = null` にした後で in-flight の
  `IndexStore.open()` が解決し、**古いストアが `store` に再代入され得る**。そのストアは
  「切り離されていない」と判定されるため、次の `invalidateStore()` まで居座る。
  fd は rename 前の inode を指し続けるので meta のオフセットと `docs.txt` の内容はずれず、
  **実害は「一世代古い証拠を返し得る」までで、見当違いのバイト列は返らない**。
  直すなら索引に世代番号を持たせ、世代が変わっていたら `openStore()` 側で代入を捨てる。
  優先度は低い（sync とツール呼び出しが同時に来た場合のみ、かつ次の sync で解消する）。
  同じ内容を `invalidateStore()` の doc コメントにも残してある（2026-08-24 記録）。
