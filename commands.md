...
```bash
# キーワードを並べても、1つの長い文として扱われるだけで分散しない
context-grill ask "決済 リトライ 冪等性 ステータス 課題 代替案" --effort deep

# 抽象的な指示は、指示文そのまま1本＋定型3本で終わる
context-grill ask "設計を見直したい" --effort deep
```

### 材料が足りないと感じたときの診断

`bundle.md` の冒頭に内訳が出ます。

```
| 提示した証拠 | 19 件 / 52340 トークン (除外 37 件) |
```
...
| 状況 | 意味 | 対処 |
| --- | --- | --- |
| 除外が多い | 候補は集まったが予算・件数で落ちた | `--effort deep`、`--source` で絞る |
| 除外が少ない | そもそも候補が集まっていない | 指示文に引用符・固有名詞を足す |

なお、**検索にかからなかったドキュメントは「除外」にも計上されません**。
索引に 862 チャンクあっても、証拠が 19 件ということは起こります。

### 複数ソースがあるときは `--source` で絞る

GitHub のコードと Confluence の資料を同じ索引に入れている場合、
両者が同じ枠を奪い合います。ドキュメント数の多い側が有利になりがちです。

```bash
# 設計資料だけを見たい
context-grill ask '「決済ステータス遷移」の設計意図を整理して' --source wiki --effort deep

# 実装だけを見たい
context-grill ask '「決済ステータス遷移」の実装を確認して' --source repo --effort deep

# 資料と実装の食い違いを見たい場合は絞らない
context-grill ask '設計資料と実装で「決済ステータス」の扱いに差がないか' --effort deep
```

### 特定のファイルやページを必ず含めたい

ファイル名・ページタイトルをそのまま指示文に書くと、そのクエリが独立して走ります。

```bash
context-grill ask 'pipeline.ts の分岐と rag.ts の閾値判定に穴がないか検証して' --effort deep
```

### 取り込めていない資料がある場合

指示文をどう書いても出てこない場合、そもそも索引に入っていない可能性があります。

- **図だけのページ**（draw.io / Gliffy など）は、本文テキストが実質タイトルのみになります。
  図の中身は取り込まれません
- **Confluence の配下ページ**は `"includeDescendants": true` を指定しないと取り込まれません
- 権限のないページ、下書きページは API から返りません

`context-grill search "ページタイトル"` でヒットするか確認すると切り分けられます。
...
```bash
context-grill ask '<何をしたいか>。「概念語1」「概念語2」「概念語3」の現状と課題を整理して。実装は <ファイル名>' \
  --task design --effort deep --source <必要なら絞る> --dry-run --out bundle.md
```

- 日本語の概念語を引用符で囲む（5〜8個が目安）
- ファイル名・識別子はそのまま書く（囲まなくてよい）
- 目的に応じて `--task` を選ぶ
- 足りなければ `--effort deep`、それでも足りなければ `--source` で絞る
