...
同梱の `setup.sh`（macOS / Linux）は、上記のステップ1〜2をまとめて実行します。

```
sh setup.sh
```

`setup.ps1.txt` は Windows 用です（メールの添付制限を避けるため拡張子を変えてあります。
使う場合は `setup.ps1` にリネームしてください）。
ただし PowerShell のスクリプト実行が禁止された環境では動きません。
**その場合は上記の手順1〜3を手で実行すれば同じことができます。スクリプトは必須ではありません。**
