...
const WS = path.join(ROOT, 'tmp-e6');
...
const cli = (label) => {
  const r = spawnSync(process.execPath, [path.join(ROOT, 'bin', 'context-grill.js'), 'sync', '--config', CFG], { encoding: 'utf8' });
  return { status: r.status, stderr末尾: String(r.stderr || '').slice(-260), stdout末尾: String(r.stdout || '').slice(-160) };
};
out.s3_別プロセスCLIsync = cli('1回目');
out.s4_別プロセスCLIsync_2回目 = cli('2回目');
out.s5_MCP側で再検索 = await call('context_grill_search', { query: 'refundPayment' });
out.s6_MCP側のsync = await call('context_grill_sync');
