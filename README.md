...
> 索引は世代番号付きで公開されるため、書き込み中と読み出し中が混ざらないようにしています。古いフォーマットの索引を開こうとした場合は `context-grill sync` で再構築してください。
...
| 埋め込みキャッシュ | 内容ハッシュ単位で永続化（provider / model / 次元数ごと）。再同期時の埋め込みコストはゼロ |
...
- **ベクトル検索（任意）**：`retrieval.embedding.provider` を `openai` / `voyage` / `openai-compat` にすると
  ...
  `dimensions` はモデルの実際の次元数に合わせてください（`openai` / `voyage` は API 側に指定できますが、
  `openai-compat` はモデルの次元がそのまま返ります。例: `nomic-embed-text` は 768）。
  食い違っている場合は、壊れた索引を作らないよう `sync` 時にエラーで止まります
...
```bash
npm test        # node --test（42 ケース）
```

内訳:

- **機能** — 検証ゲート、チャンクの行番号整合、クエリ計画の決定性、証拠パックの再現性、埋め込みのリクエスト組み立てとレスポンス順序・次元数検査、埋め込みキャッシュの区別と途中失敗時の永続化
